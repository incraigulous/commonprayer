...
export default function TextBlock({ doc }: TextBlockProps) {
  ...
  const paragraphClass = 'text-ink leading-relaxed mb-2 font-serif block'
  ...
  const responseNode = response && (
    <Text className={`${paragraphClass} font-bold mt-1`} style={paragraphStyle}>{inlineRuns(response)}</Text>
  )

  // "The Blessing" — the office's closing benediction — antiphons (a
  // refrain framing a psalm/canticle, sung before and after it), and collects
  // are featured liturgical moments, not routine body text: set apart in
  // Callout, same as the ui-kit's own DailyOffice template and Callout story
  // examples.
  if (style === 'blessing') {
    return (
  ...
  if (style === 'collect') {
    return (
      <Callout variant="prayer" title={title || 'A Collect'} className="my-4">
        {paragraphs}
        {responseNode}
      </Callout>
    )
